...
export default function CheckedMedicine({ Ltoken }) {
        ...
        try {
            ...
            setMedicines(prevState => ({
              ...
              [timeOfDay]: prevState[timeOfDay].map(item =>
                item.scheduleMedicineId === med.scheduleMedicineId ? { ...item, taken: true } : item
              )
            }));
        } catch (error) {
    ...
    const bgColor = timeOfDay === 'morning' ? 'bg-[#ff8540]' : timeOfDay === 'lunch' ? 'bg-[#3b9ffc]' : 'bg-[#3f72c1]';
    const textColor = timeOfDay === 'morning' ? 'text-[#ff8540]' : timeOfDay === 'lunch' ? 'text-[#3b9ffc]' : 'text-[#3f72c1]';
    ...
    return (
        ...
        >
            <h2 className={`text-xl mb-2 text-white`}>{timeOfDay === 'morning' ? '아침' : timeOfDay === 'lunch' ? '점심' : '저녁'}</h2>
            <span>{med.title}</span> 
            <button
              className={`w-full bg-white rounded-lg ${textColor} px-4 py-2 mt-5 font-normal`}
              onClick={() => handleTakeMedicine(med, timeOfDay)}
    ...
    {activeTab === 'tab1' && !isFormSubmitted && (
        <div className="grid justify-items-center">
          <form onSubmit={handleFormSubmit} className="flex flex-col w-auto p-4">
            {plusMedicines.map((plusMedicines, index) => (
              <div key={index} className="mb-4 border p-4 rounded-lg">
                <div className="mb-4">
                  <label className="flex block text-gray-700 pb-5">약 이름</label>
                  <input
                    type="text"
                    name="title"
                ...
                <div className="flex items-center mb-4">
                  <label className="px-3 block text-gray-700">아침</label>
                  <input
                    type="checkbox"
                    name="morning"
                ...
                <div className="flex items-center mb-4">
                  <label className="px-3 block text-gray-700">점심</label>
                  <input
                    type="checkbox"
                    name="lunch"
                ...
                <div className="flex items-center mb-4">
                  <label className="px-3 block text-gray-700">저녁</label>
                  <input
                    type="checkbox"
                    name="dinner"
            ...
            <button type="button" onClick={addNewForm} className="w-full px-4 py-2 bg-blue-500 text-white rounded-lg mt-4">
              추가하기
            </button>
            <button type="submit" className="w-full px-4 py-2 bg-blue-500 text-white rounded-lg mt-4">
              등록 완료!
            </button>
          </form>
        </div>
